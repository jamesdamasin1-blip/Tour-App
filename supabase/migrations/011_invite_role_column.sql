-- Migration 011: Add 'role' column to trip_invites so inviters can choose
-- between 'editor' (can edit/modify/delete) and 'viewer' (read-only).
-- The accept_trip_invite RPC now reads this column instead of hardcoding 'editor'.

-- Add role column with default 'editor' for backward compatibility
ALTER TABLE trip_invites
    ADD COLUMN IF NOT EXISTS role TEXT NOT NULL DEFAULT 'editor'
    CHECK (role IN ('editor', 'viewer'));

-- Update the accept_trip_invite RPC to use the invite's role
