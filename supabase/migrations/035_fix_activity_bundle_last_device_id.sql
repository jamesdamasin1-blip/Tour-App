-- Migration 035: ensure activity bundle saves stamp the current device id.
-- This prevents realtime consumers from misclassifying cross-device edits as
-- self-emitted when an activity keeps an old creator/member device id.
...
DECLARE
    ...
    v_last_device_id TEXT := NULLIF(p_activity->>'lastDeviceId', '');
...
BEGIN
    ...
    IF v_should_replace_expenses THEN
        ...
        LOOP
            ...
            IF FOUND THEN
                ...
                SET lots = public.restore_wallet_lots_json(
                    ...
                    last_device_id = COALESCE(v_last_device_id, v_wallet_row.last_device_id)
        ...
        SET deleted_at = to_timestamp(v_last_modified / 1000.0),
            updated_at = v_last_modified,
            updated_by = v_caller_id,
            last_device_id = COALESCE(v_last_device_id, public.expenses.last_device_id)
        WHERE activity_id = v_activity_id
          AND deleted_at IS NULL;
    END IF;
    ...
    SET trip_id = EXCLUDED.trip_id,
        ...
        last_device_id = COALESCE(EXCLUDED.last_device_id, public.activities.last_device_id),
        deleted_at = NULL;
    ...
    IF v_should_replace_expenses THEN
        ...
        LOOP
            ...
            VALUES (
                ...
                v_last_modified,
                v_caller_id,
                COALESCE(NULLIF(v_expense->>'lastDeviceId', ''), v_last_device_id),
                v_caller_id,
                NULL
            )
            ON CONFLICT (id) DO UPDATE
            SET trip_id = EXCLUDED.trip_id,
                ...
                updated_at = EXCLUDED.updated_at,
                updated_by = EXCLUDED.updated_by,
                last_device_id = COALESCE(EXCLUDED.last_device_id, public.expenses.last_device_id),
                deleted_at = NULL;
        END LOOP;
    END IF;
