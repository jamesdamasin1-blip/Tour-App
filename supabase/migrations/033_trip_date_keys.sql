...
ADD COLUMN IF NOT EXISTS start_date_key TEXT;

ALTER TABLE public.trips
ADD COLUMN IF NOT EXISTS end_date_key TEXT;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'trips_start_date_key_check'
    ) THEN
        ALTER TABLE public.trips
        ADD CONSTRAINT trips_start_date_key_check
        CHECK (start_date_key IS NULL OR start_date_key ~ '^\d{4}-\d{2}-\d{2}$');
    END IF;

    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'trips_end_date_key_check'
    ) THEN
        ALTER TABLE public.trips
        ADD CONSTRAINT trips_end_date_key_check
        CHECK (end_date_key IS NULL OR end_date_key ~ '^\d{4}-\d{2}-\d{2}$');
    END IF;
END $$;

CREATE OR REPLACE FUNCTION public.trip_country_timezone(
    p_country TEXT
)
RETURNS TEXT
LANGUAGE sql
IMMUTABLE
AS $$
    SELECT CASE COALESCE(p_country, '')
...
CREATE OR REPLACE FUNCTION public.trip_date_key_from_timestamp(
    p_timestamp BIGINT,
    p_country TEXT
)
RETURNS TEXT
...
AS $$
    SELECT CASE
        WHEN p_timestamp IS NULL OR p_timestamp <= 0 THEN NULL
        ELSE to_char(
            to_timestamp(p_timestamp / 1000.0) AT TIME ZONE public.trip_country_timezone(p_country),
            'YYYY-MM-DD'
        )
    END;
$$;
...
UPDATE public.trips
SET start_date_key = COALESCE(start_date_key, public.trip_date_key_from_timestamp(start_date, home_country)),
    end_date_key = COALESCE(end_date_key, public.trip_date_key_from_timestamp(end_date, home_country))
WHERE deleted_at IS NULL
  AND (start_date_key IS NULL OR end_date_key IS NULL);
...
DECLARE
    ...
    v_home_country TEXT := p_trip->>'homeCountry';
    v_start_date BIGINT := NULLIF(p_trip->>'startDate', '')::BIGINT;
    v_end_date BIGINT := NULLIF(p_trip->>'endDate', '')::BIGINT;
    v_start_date_key TEXT := COALESCE(
        NULLIF(p_trip->>'startDateKey', ''),
        public.trip_date_key_from_timestamp(v_start_date, v_home_country)
    );
    v_end_date_key TEXT := COALESCE(
        NULLIF(p_trip->>'endDateKey', ''),
        public.trip_date_key_from_timestamp(v_end_date, v_home_country)
    );
BEGIN
...
DECLARE
    ...
    v_next_home_country TEXT;
    v_next_start_date BIGINT;
    v_next_end_date BIGINT;
    v_next_start_date_key TEXT;
    v_next_end_date_key TEXT;
BEGIN
    IF v_caller_id IS NULL THEN
        RAISE EXCEPTION 'Not authenticated';
    END IF;
    ...
    v_next_home_country := COALESCE(p_trip->>'homeCountry', v_trip.home_country);
    v_next_start_date := COALESCE(NULLIF(p_trip->>'startDate', '')::BIGINT, v_trip.start_date);
    v_next_end_date := COALESCE(NULLIF(p_trip->>'endDate', '')::BIGINT, v_trip.end_date);
    v_next_start_date_key := COALESCE(
        NULLIF(p_trip->>'startDateKey', ''),
        v_trip.start_date_key,
        public.trip_date_key_from_timestamp(v_next_start_date, v_next_home_country)
    );
    v_next_end_date_key := COALESCE(
        NULLIF(p_trip->>'endDateKey', ''),
        v_trip.end_date_key,
        public.trip_date_key_from_timestamp(v_next_end_date, v_next_home_country)
    );
    ...
    SET title = COALESCE(p_trip->>'title', v_trip.title),
        ...
        start_date_key = v_next_start_date_key,
        end_date_key = v_next_end_date_key,
        home_country = v_next_home_country,
...
GRANT EXECUTE ON FUNCTION public.trip_country_timezone(TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION public.trip_date_key_from_timestamp(BIGINT, TEXT) TO authenticated;
