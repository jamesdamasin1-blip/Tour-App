-- Migration 025: Cloud-first trip bundle RPCs
-- Makes trip create/update wallet flows transactional so trips and wallets
-- cannot drift apart when a mid-sequence write fails.
...
CREATE OR REPLACE FUNCTION public.build_trip_wallet_mirror(
    p_trip_id TEXT,
    p_wallet_source JSONB,
...
AS $$
DECLARE
    v_wallet_source JSONB := COALESCE(p_wallet_source, '[]'::JSONB);
    v_wallets_mirror JSONB;
BEGIN
    IF jsonb_typeof(v_wallet_source) IS DISTINCT FROM 'array' THEN
        v_wallet_source := '[]'::JSONB;
    END IF;
    ...
    SELECT COALESCE(
        jsonb_agg(
            jsonb_build_object(
                'id', w.id,
                'tripId', w.trip_id,
                'country', COALESCE(source_wallet.value->>'country', ''),
                'currency', w.currency,
                ...
                'baselineSource', NULLIF(source_wallet.value->>'baselineSource', ''),
                'lots', COALESCE(w.lots, '[]'::JSONB),
                'createdAt', COALESCE(NULLIF(source_wallet.value->>'createdAt', '')::BIGINT, p_last_modified),
            ...
            ORDER BY COALESCE(NULLIF(source_wallet.value->>'createdAt', '')::BIGINT, p_last_modified), w.id
        ),
        '[]'::JSONB
    )
    ...
    LEFT JOIN LATERAL (
        SELECT value
        FROM jsonb_array_elements(v_wallet_source) value
        WHERE value->>'id' = w.id
        LIMIT 1
    ) source_wallet ON TRUE
...
GRANT EXECUTE ON FUNCTION public.build_trip_wallet_mirror(TEXT, JSONB, BIGINT) TO authenticated;
