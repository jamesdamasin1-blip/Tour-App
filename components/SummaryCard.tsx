...
export const SummaryCard = React.memo(({ spentAmount, totalAmount }: SummaryCardProps) => {
    ...
    const percentage = useMemo(() => {
        ...
        const pct = (spentAmount / totalAmount) * 100;
    ...
    return (
        ...
        >
            ...
            >
                ...
                <Text className={`text-[10px] font-black mb-3 uppercase tracking-widest opacity-60 ${isDark ? 'text-[#9EB294]' : 'text-[#5D6D54]'}`}>Your Spending</Text>

                    <View className="flex-row items-end justify-between mb-5">
                    <View className="flex-row items-baseline">
                        <Text className={`text-4xl font-black leading-none ${isDark ? 'text-[#F2F0E8]' : 'text-gray-900'}`}>
                            ₱{spentAmount.toLocaleString()}
                        </Text>
                        <Text className={`text-sm font-bold ml-1.5 opacity-80 ${isDark ? 'text-[#9EB294]' : 'text-gray-400'}`}>
                    ...
                    >
                        <Text className={`text-xs font-black ${isOverBudget ? 'text-red-500' : 'text-[#9EB294]'}`}>
                            {percentage}%
                        </Text>
                    </View>
                </View>
                ...
                >
                    <Text style={{ color: isDark ? '#B2C4AA' : '#5D6D54', fontWeight: '900', fontSize: 13, marginRight: 8, letterSpacing: -0.3 }}>VIEW BUDGET ANALYSIS</Text>
